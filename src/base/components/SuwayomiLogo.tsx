...
// TODO: Replace with custom logo images when available
// Expanded: place full logo image at /public/logo-expanded.svg (or .png)
// Collapsed: place icon-only image at /public/logo-collapsed.svg (or .png)
// Then swap SuwayomiIcon/Typography below with <img> tags

function SuwayomiIcon({ size = 36 }: { size?: number }) {
    const theme = useTheme();
    const isDark = theme.palette.mode === 'dark';
    // Dark theme:  ring = primary.light, inner = primary.dark, letters = white.
    // Light theme: ring = primary.dark,  inner = primary.light, letters = white.
    // The ring always uses the lighter shade, inner always darker, so the
    // mark reads the same way on both themes (bright halo + dark disk).
    const ringColor = isDark ? theme.palette.primary.light : theme.palette.primary.dark;
    const innerColor = isDark ? theme.palette.primary.dark : theme.palette.primary.light;
    const letterColor = theme.palette.common.white;

    return (
        <svg width={size} height={size} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <circle cx="50" cy="50" r="50" fill={ringColor} />
            <circle cx="50" cy="50" r="39" fill={innerColor} />
            <path
...
export function SuwayomiLogo({ collapsed = false }: { collapsed?: boolean }) {
    return (
        <Box
            sx={{
                display: 'flex',
